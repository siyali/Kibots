    ...
    @objc func longPressSet(longPressGestureRecognizer : UILongPressGestureRecognizer) {
        if longPressGestureRecognizer.state == UIGestureRecognizerState.began {
            ...
            let confirmAction = UIAlertAction(title: "Confirm", style: .default) { (_) in
                ...
                if Functionalities.tappedOperation == "Holding"{
                    let pressed_food = Functionalities.holdingItems[rowIndex]
                    if max_field.text != ""{
                        if let max_int = Int(max_field.text!){
                            max_temp = max_int
                            Functionalities.myUser?.addHoldingItemMax(station: Functionalities.tappedKitchenHP!, fooditem: pressed_food, max: max_temp)
                        }
                    }
                    
                    if min_field.text != ""{
                        if let min_int = Int(min_field.text!){
                            min_temp = min_int
                            Functionalities.myUser?.addHoldingItemMin(station: Functionalities.tappedKitchenHP!, fooditem: pressed_food, min: min_temp)
                        }
                    }
                    minmax = (min_temp, max_temp)
                    
                    Functionalities.minMaxMap[Functionalities.holdingItems[rowIndex]] = minmax
                    Functionalities.myUser?.updateMinMaxTemp(hpr: Functionalities.tappedOperation!, station: Functionalities.tappedKitchenHP!, fooditem: Functionalities.holdingItems[rowIndex], min: min_temp, max: max_temp)
                    print("minmaxmap updated")
                    print(Functionalities.minMaxMap)
                }else if Functionalities.tappedOperation == "Production"{
                    let pressed_food = Functionalities.productionItems[rowIndex]
                    if max_field.text != ""{
                        if let max_int = Int(max_field.text!){
                            max_temp = max_int
                            Functionalities.myUser?.addProductionItemMax(station: Functionalities.tappedKitchenHP!, fooditem: pressed_food, max: max_temp)
                        }
                    }
                    ...
                    if min_field.text != ""{
                        if let min_int = Int(min_field.text!){
                            min_temp = min_int
                            Functionalities.myUser?.addProductionItemMin(station: Functionalities.tappedKitchenHP!, fooditem: pressed_food, min: min_temp)
                        }
                    }
                    minmax = (min_temp, max_temp)
                    Functionalities.minMaxMap[Functionalities.productionItems[rowIndex]] = minmax
                    Functionalities.myUser?.updateMinMaxTemp(hpr: Functionalities.tappedOperation!, station: Functionalities.tappedKitchenHP!, fooditem: Functionalities.productionItems[rowIndex], min: min_temp, max: max_temp)
                }
        ...
        if (editingStyle == .delete) {
           ...
           // tableView.deleteRows(at: [indexPath as IndexPath], with: .automatic)
            if Functionalities.tappedOperation == "Holding"{
                ...
                Functionalities.myUser?.updateProductionWithMinMax(station: Functionalities.tappedKitchenHP!, foodItems: Functionalities.productionItems, minmaxMap: Functionalities.minMaxMap)
            }
            self.tableView.reloadData()
        }
